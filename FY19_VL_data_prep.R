...
psnu_im_all <- fread('MER_Structured_Dataset_PSNU_IM_FY17-18_20181221_v2_1.txt')
...
write.xlsx(ret, 'ret_age_sex.xlsx')
...
#   summarise_at(vars(FY2017Q1:FY2019Q1), sum, na.rm=T) %>% 
#   ungroup()
...
pvls <- psnu_im_all %>% filter(indicator=='TX_PVLS', standardizedDisaggregate %in% pvls_std_disagg,
  ...
  select(-coarseDisaggregate:-isMCAD, -indicatorType, -AgeFine,-AgeCoarse,-AgeSemiFine,
         -MechanismID, -MechanismUID) %>% 
  group_by_at(vars(Region:otherDisaggregate)) %>% 
  summarise_at(vars(FY2018Q4:FY2019Q1), sum, na.rm=T) %>% 
  ungroup() %>% 
  filter_at(vars(FY2018Q4:FY2019Q1), any_vars(.!=0))

new <- psnu_im_all %>% filter(indicator == 'TX_NEW', standardizedDisaggregate %in% new_std_disagg,
                              otherDisaggregate %in% new_oth_disagg) %>%
  select(-coarseDisaggregate:-isMCAD, -indicatorType, -AgeFine,-AgeCoarse,
         -MechanismID, -MechanismUID, -FY2017_TARGETS, -AgeSemiFine) %>% 
  group_by_at(vars(Region:otherDisaggregate)) %>% 
  summarise_at(vars(FY2017Q1:FY2019Q1), sum, na.rm=T) %>% 
  ungroup()%>% 
  filter_at(vars(FY2017Q1:FY2019Q1), any_vars(.!=0))

curr <- psnu_im_all %>% filter(indicator == 'TX_CURR', standardizedDisaggregate %in% curr_std_disagg) %>%
  select(-coarseDisaggregate:-isMCAD, -indicatorType, -AgeFine,-AgeCoarse,-otherDisaggregate,-AgeSemiFine,
         -MechanismID, -MechanismUID) %>% 
  group_by_at(vars(Region:Sex)) %>% 
  summarise_at(vars(FY2018Q3:FY2019Q1), sum, na.rm=T) %>% 
  ungroup()%>% 
  filter_at(vars(FY2018Q3:FY2019Q1), any_vars(.!=0))
...
stock <- psnu_im_all %>% filter(indicator == 'SC_STOCK', standardizedDisaggregate %in% stock_std_disagg, 
  ...
  select(-coarseDisaggregate:-isMCAD, -indicatorType, -AgeFine,-AgeCoarse,
         -MechanismID, -MechanismUID) %>% 
  group_by_at(vars(Region:otherDisaggregate)) %>% 
  summarise_at(vars(FY2017_TARGETS:FY2019Q1), sum, na.rm=T) %>% 
  ungroup()
...
pmtct <- psnu_im_all %>% filter(indicator == 'PMTCT_ART', standardizedDisaggregate %in% pmtct_std_disagg, 
  ...
  select(-coarseDisaggregate:-isMCAD, -indicatorType, -AgeFine,-AgeCoarse,-AgeSemiFine,
         -MechanismID, -MechanismUID) %>% 
  group_by_at(vars(Region:otherDisaggregate)) %>% 
  summarise_at(vars(FY2017Q4:FY2019Q1), sum, na.rm=T) %>% 
  ungroup()%>% 
  filter_at(vars(FY2017Q4:FY2019Q1), any_vars(.!=0))
...
site_im <- Site_IM_MSD_FY19Q1_02_27 %>% filter(indicator %in% c('TX_CURR', 'TX_NEW', 'TX_PVLS', 'LAB_PTCQI', 
                                                   'LAB_PTCQI_MIL', 'SC_STOCK', 'PMTCT_ART'))

saveRDS(site_im, 'site_im_02_27.rds')
...
# psnu_im2 <- psnu_im.vl %>% 
#   gather(period, val, FY2017_TARGETS:FY2019Q1) %>% 
...
#check site count numbers
tab1 <- site_cal_indi2 %>% group_by(OperatingUnit,PSNU) %>% 
  summarise_at(vars(site_curr_reporting:site_vl_supp_report),sum,na.rm=T) 
...
site.count <- site_im %>% group_by(OperatingUnit, SNU1, PSNU,PSNUuid,
                                   SNUPrioritization, ImplementingMechanismName, PrimePartner, FundingAgency) %>% 
  summarise(facility_count=n_distinct(SiteName)) %>% 
